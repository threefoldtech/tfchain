...
# GNU sed and BSD sed (macOS) disagree on in-place editing: BSD requires an argument
# to -i, GNU must not have one. Detect once and use $(SED_I) throughout.
...
version-bump:
	...
	if [ "$(type)" = "patch" ] || [ "$(type)" = "minor" ] || [ "$(type)" = "major" ]; then \
		...
		current_spec_version=$$(sed -n -e 's/^.*spec_version: \([0-9][0-9]*\),$$/\1/p' substrate-node/runtime/src/lib.rs); \
		if [ -z "$${retain_spec_version}" ]; then \
			...
			$(SED_I) "s/spec_version: $$current_spec_version,/spec_version: $$new_spec_version,/" substrate-node/runtime/src/lib.rs; \
		fi; \
		...
		$(SED_I) "s/^version = .*/version = \"$$new_version\"/" substrate-node/Cargo.toml; \
		$(SED_I) "s/^version: .*/version: $$new_version/" substrate-node/charts/substrate-node/Chart.yaml; \
		$(SED_I) "s/^appVersion: .*/appVersion: '$$new_version'/" substrate-node/charts/substrate-node/Chart.yaml; \
		$(SED_I) "s/^version: .*/version: $$new_version/" bridge/tfchain_bridge/chart/tfchainbridge/Chart.yaml; \
		$(SED_I) "s/^appVersion: .*/appVersion: '$$new_version'/" bridge/tfchain_bridge/chart/tfchainbridge/Chart.yaml; \
		$(SED_I) "s/^version: .*/version: $$new_version/" activation-service/helm/tfchainactivationservice/Chart.yaml; \
		$(SED_I) "s/^appVersion: .*/appVersion: '$$new_version'/" activation-service/helm/tfchainactivationservice/Chart.yaml; \
