...
fn test_node_contract_billing_cycles_delete_node_cancels_contract() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            our_events[9],
            record(MockEvent::SmartContractModule(SmartContractEvent::<TestRuntime>::IPsFreed(1, ips)))
        );
        assert_eq!(
            our_events[9],
            record(MockEvent::SmartContractModule(SmartContractEvent::<TestRuntime>::NodeContractCanceled(1, 1, 2)))
        );
    });
}
...
fn test_node_contract_only_public_ip_billing_cycles() {
    new_test_ext().execute_with(|| {
        ...
        let (amount_due_as_u128, discount_received) = calculate_tft_cost(contract_id, twin_id, 10);
        run_to_block(22);
        check_report_cost(1, 2, amount_due_as_u128, 22, discount_received);

        let (amount_due_as_u128, discount_received) = calculate_tft_cost(contract_id, twin_id, 10);
        run_to_block(32);
        check_report_cost(1, 3, amount_due_as_u128, 32, discount_received);

        let (amount_due_as_u128, discount_received) = calculate_tft_cost(contract_id, twin_id, 10);
        run_to_block(42);
        check_report_cost(1, 4, amount_due_as_u128, 42, discount_received);

        let (amount_due_as_u128, discount_received) = calculate_tft_cost(contract_id, twin_id, 10);
        run_to_block(52);
        check_report_cost(1, 5, amount_due_as_u128, 52, discount_received);
    });
}
...
fn test_node_contract_out_of_funds_should_move_state_to_graceperiod_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            our_events[3],
            record(MockEvent::SmartContractModule(SmartContractEvent::<TestRuntime>::ContractGracePeriodStarted(1, 1, 3, 21)))
        );
    });
}
...
fn test_name_contract_billing() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            our_events[1],
            record(MockEvent::SmartContractModule(SmartContractEvent::<TestRuntime>::ContractBilled(contract_bill_event)))
        );
    });
}
...
fn test_rent_contract_canceled_due_to_out_of_funds_should_cancel_node_contracts_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            our_events[15],
            record(MockEvent::SmartContractModule(SmartContractEvent::<TestRuntime>::NodeContractCanceled(2, 1, 3)))
        );
        assert_eq!(
            our_events[16],
            record(MockEvent::SmartContractModule(SmartContractEvent::<TestRuntime>::RentContractCanceled(1)))
        );
    });
}
...
fn test_restore_rent_contract_in_grace_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            our_events[1],
            record(MockEvent::SmartContractModule(SmartContractEvent::<TestRuntime>::ContractGracePeriodStarted(1, 1, 3, 11)))
        );
...
fn test_rent_contract_and_node_contract_canceled_when_node_is_deleted_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            our_events[4],
            record(MockEvent::SmartContractModule(SmartContractEvent::<TestRuntime>::NodeContractCanceled(2, 1, 2)))
        );
        assert_eq!(
            our_events[7],
            record(MockEvent::SmartContractModule(SmartContractEvent::<TestRuntime>::RentContractCanceled(1)))
