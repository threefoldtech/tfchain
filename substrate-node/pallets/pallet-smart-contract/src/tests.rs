...
fn test_create_group_then_capacity_reservation_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(SmartContractModule::group_create(Origin::signed(alice())));
...
fn test_remove_group_unauthorized_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            SmartContractModule::group_delete(Origin::signed(bob()), 1),
...
fn test_remove_group_active_capacity_reservation_contracts_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            SmartContractModule::group_delete(Origin::signed(alice()), 1),
...
fn test_create_capacity_contract_reservation_finding_node_using_group() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            SmartContractModule::capacity_reservation_resources(4),
            ConsumableResources {
                total_resources: resources_c1(),
                used_resources: ResourcesInput::empty(),
            }
        );
        assert_eq!(SmartContractModule::active_deployments(4).len(), 0);
        assert_eq!(
            TfgridModule::node_resources(1).used_resources,
...
fn test_capacity_reservation_contract_with_policy_any_and_features_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            SmartContractModule::capacity_reservation_resources(1),
            ConsumableResources {
        ...
        assert_eq!(TfgridModule::node_power(3).target, PowerTarget::Up);
    });
}
...
fn test_capacity_reservation_contract_create_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(TfgridModule::node_power(1).target, PowerTarget::Up,)
    });
}
...
fn test_capacity_reservation_contract_create_reserving_full_node_then_deployment_contract_then_cancel_everything(
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(TfgridModule::node_power(node_id).target, PowerTarget::Up);
        assert_eq!(
            TfgridModule::node_resources(node_id).used_resources,
            TfgridModule::node_resources(node_id).total_resources
        );
        ...
        assert_eq!(SmartContractModule::active_deployments(1), vec![1]);
        ...
        assert_eq!(TfgridModule::node_power(node_id).target, PowerTarget::Down);
        assert_eq!(
...
fn test_cancel_capacity_reservation_contract_should_not_shutdown_first_node() {
    new_test_ext().execute_with(|| {
        run_to_block(1, None);
        prepare_farm_with_three_nodes();
        assert_eq!(TfgridModule::node_power(1).target, PowerTarget::Down);
        assert_ok!(SmartContractModule::contract_capacity_reservation_create(
            Origin::signed(bob()),
            1,
            CapacityReservationPolicy::Any {
...
fn test_cancel_capacity_reservation_contract_shutdown_node() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(TfgridModule::node_power(3).target, PowerTarget::Down);
        ...
        assert_eq!(
            TfgridModule::node_resources(2).used_resources,
...
fn test_deployment_cancel_contract_not_exists_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            SmartContractModule::contract_cancel(Origin::signed(alice()), 1),
...
fn test_deployment_cancel_contract_wrong_twins_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            SmartContractModule::deployment_cancel(Origin::signed(bob()), 1),
            Error::<TestRuntime>::TwinNotAuthorized
        );
    });
}
...
#[test]
fn test_deployment_update_decrease_resources_fails() {
    new_test_ext().execute_with(|| {
        run_to_block(1, None);
        prepare_farm_and_node();
        ...
        assert_noop!(
            ...
            Error::<TestRuntime>::NotAllowedToReduceResources,
        );
    });
}
...
fn test_cancel_capacity_reservation_contract_with_active_deployment_contracts_fails() {
    ...
    ext.execute_with(|| {
        ...
        assert_noop!(
            SmartContractModule::contract_cancel(Origin::signed(bob()), 1,),
...
pub fn create_capacity_reservation_and_add_to_group(
    ...
    assert_eq!(
        SmartContractModule::active_deployments(cnt_contracts + 1).len(),
        0
    );
    assert_eq!(
        SmartContractModule::capacity_reservation_resources(cnt_contracts + 1),
        ConsumableResources {
    ...
    assert_eq!(
        TfgridModule::node_power(expected_node_id).target,
        PowerTarget::Up
    );
...
fn prepare_farm_three_nodes_three_capacity_reservation_contracts() {
    ...
    assert_eq!(
        SmartContractModule::capacity_reservation_resources(2),
        ConsumableResources {
    ...
    assert_eq!(
        SmartContractModule::capacity_reservation_resources(3),
        ConsumableResources {
            total_resources: resources_c3(),
            used_resources: ResourcesInput::empty(),
        }
