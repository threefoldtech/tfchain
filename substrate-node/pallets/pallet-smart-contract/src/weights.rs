...
impl<T: frame_system::Config> WeightInfo for SubstrateWeight<T> {
	...
	/// Placeholder until regenerated by `060_generate_benchmark_weights.yml`.
	/// Base is `cancel_contract_collective` (closest read/write profile: contract +
	/// billing + two node-keyed maps). `n` is the max length of the source and
	/// destination `ActiveNodeContracts` vectors, both of which are read, mutated
	/// and re-encoded; the per-element slope is modelled on `pallet-tfgrid`'s
	/// `add_twin_admin`, scaled from its 32-byte elements to our 8-byte u64s.
	fn migrate_node_contract(n: u32, ) -> Weight {
		// Proof Size summary in bytes:
		//  Measured:  `1650 + n * (8 ±0)`
		//  Estimated: `7590 + n * (8 ±0)`
		Weight::from_parts(233_363_000, 7590)
			.saturating_add(Weight::from_parts(53_223, 0).saturating_mul(n.into()))
