...
def setup_first_node():
    substrate = SubstrateInterface(url="ws://127.0.0.1:9945", ss58_format=42, type_registry_preset='polkadot')
    ...
    call_create_node = substrate.compose_call("TfgridModule", "create_node", { "farm_id": "1", "resources": { "hru": 1024 * GIGABYTE,"sru": 512 * GIGABYTE,"cru": 8,"mru": 16 * GIGABYTE},"location": {"longitude": "garbage","latitude": "garbage"},"country": "Belgium","city": "Ghent","interfaces": [],"secure_boot": False,"virtualized": False,"serial_number": "garbage"})
    ...
    call_create_contract = substrate.compose_call("SmartContractModule","create_node_contract",{"node_id": 1,"deployment_data": randbytes(32),"deployment_hash": randbytes(32),"public_ips": 0,"solution_provider_id": None})
    ...
    call_create_contract = substrate.compose_call("SmartContractModule","report_contract_resources",{"contract_resources": [{"contract_id": 1,"used": {"hru": 0,"sru": 20 * GIGABYTE,"cru": 2,"mru": 4 * GIGABYTE}}]})
    call_create_contract_signed = substrate.create_signed_extrinsic(call_create_contract, key_alice)
    response = substrate.submit_extrinsic(call_create_contract_signed, wait_for_finalization=True)
    if response.error_message:
        print(response.error_message)    

def setup_second_node():
    GIGABYTE = 1024*1024*1024
