...
// Reset clears all withdraw and refund state. The store keys are chain-scoped
// (withdraw tx ids restart from a low number after a chain reset and would collide
// with stale COMPLETED entries, causing new withdraws to be wrongly skipped), so the
// store must be wiped whenever the underlying chain/bridge account is reset. This is
// invoked when the bridge is started with the rescan/fresh-start flag, alongside the
// Stellar cursor reset.
func (s *IdempotencyStore) Reset() error {
	return s.db.Update(func(tx *bolt.Tx) error {
		for _, bucket := range [][]byte{bucketWithdraw, bucketRefund} {
